...
11. Every session finishes with a short down-regulation close to shift the athlete out of sympathetic drive — see the down-regulation rules below. It sits OUTSIDE the time budget: its minutes are NOT part of the 12-minute warm-up or the 48 minutes of work, the blocks still sum to 48, and you must never trim a work block to make room for it.
12. Balance session intensity: do NOT water every movement down to a light load. The main movement is Heavy per rule 7; let accessories and conditioning pieces sit Moderate or Light so the session's loading averages out around Moderate overall, with deliberate variation. A session where every trained pattern comes out Light is a failed adaptation unless the ORIGINAL WOD was explicitly a light/recovery day — if trimming for time or safety substitutions have pushed everything down, raise the load or drop the reps on the main movement(s) to restore the intended stimulus. Olympic lifts are the one exception: they stay technique-light per rules 3 and 8, so bring the intensity through the other patterns that day.
13. Oly-prep prefix: every session's warm-up begins with a fixed, coach-prescribed sequence — see the Oly-prep prefix rules below. It applies to every session (not just Olympic-lift days) and must be reproduced exactly as written; never reorder, symmetrize, or drop any part of it.

Oly-prep prefix rules (fixed — prescribed by the athlete's Olympic lifting coach, reproduce verbatim, every session, always first in the warm-up):
...
4. Empty bar overhead squats, 8 reps
5. Empty bar snatch cycles, 5 reps
6. Empty bar snatches, 5 reps
...
- Group into exactly 3 warm-up bullets, in this order: (1) the three stretches together, (2) the overhead squats, (3) the snatch cycles and snatches together.
- This prefix is fixed and mandatory — it is never trimmed, reordered, or treated as optional, regardless of what the original WOD contains.

Warm-up rules:
- The oly-prep prefix (3 fixed bullets, above) always comes first, verbatim, in every session.
- After the prefix, build day-specific warm-up content from the specific movements that appear in the day's blocks (e.g. if the session has back squats, include bodyweight squats and empty-bar/light-loaded squats; if it has a barbell hinge, include glute bridges and light RDLs; if it has a snatch or clean, include the relevant barbell progression drills; if it has a gymnastics or engine piece, include the relevant prep for that too).
- Every day-specific warm-up line must be a specific, prescribed movement with an exact rep count, distance, or duration — never a vague label. Do not write generic entries like "dynamic mobility", "general warm-up", "activation", or "movement prep" with no detail.
- Keep the day-specific portion short: at most 2 more bullets after the fixed prefix (5 total), each containing ONE movement (or a tight superset of two at most). Do not turn the warm-up into a checklist of 8-10 exercises — pick the few that prepare the day's main lifts and skip the rest. A barbell ramp set counts as one bullet.
- Bad: "4 min dynamic mobility: hips, thoracic, shoulders, ankles"
- Bad: the fixed prefix followed by five more day-specific bullets covering jogging, leg swings, lunges, squat holds, and a barbell ramp — too many steps for 12 minutes
- Good (complete example of the right size, for a back squat + wall ball day):
  ...
  - "Empty bar: 8 overhead squats"
  - "Empty bar: 5 snatch cycles, 5 snatches"
...
## Warm-up (12 min)
- bullet list of warm-up items: the 3 fixed oly-prep prefix bullets first (verbatim, every session), then at most 2 day-specific bullets, each one a specific movement with an exact rep/distance/duration count as described above

## Block 1: <name> (~X min)
Sets/reps/loading, rest, and a one-line intent note. Repeat "## Block N" for each block. Time estimates must sum to 48 minutes.

## Down-regulation close (~X min, on top of the 60)
- 2-3 bullets as described in the down-regulation rules above. State its duration in the heading, and do NOT add those minutes to the 60-minute session or to any block's time estimate.
